...
int resource_manager_init(struct ResourceManager* resource_manager, struct GPUAPI* gpu_api) {
  ...
  resource_manager->music_clip_cache = calloc(1, sizeof(struct AudioClipCache));
  resource_manager->fart_clip_cache = calloc(1, sizeof(struct AudioClipCache));
...
void resource_manager_delete(struct ResourceManager* resource_manager) {
  ...
  free(resource_manager->music_clip_cache);
  free(resource_manager->fart_clip_cache);
}
